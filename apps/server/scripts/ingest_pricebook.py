...
class PricebookVectorStoreService:
    ...
    async def get_or_create_vector_store(self) -> str:
        ...
        try:
            logger.info(f"Creating new vector store: {PRICEBOOK_VECTOR_STORE_NAME}")
    ...
    async def upload_pricebook_file(self, file_path: Path) -> str:
        ...
        try:
            ...
            if existing_file_id:
                logger.info(f"Found existing file {existing_file_id}, deleting...")
            ...
            logger.info(f"Uploaded file: {uploaded_file.id} ({filename})")
            ...
            logger.info(f"Attached file {uploaded_file.id} to vector store {vector_store_id}")

            return uploaded_file.id

        except Exception as e:
            logger.error(f"Failed to upload pricebook file: {e}")
            raise
...
async def ingest_pricebook():
    ...
    if not pricebook_path.exists():
        logger.error(f"Pricebook file not found: {pricebook_path}")
        print(f"\n❌ Error: {pricebook_path} not found")
        print("Run scripts/clean_pricebook.py first to generate the cleaned file.")
        return

    logger.info("=" * 60)
    logger.info("PRICEBOOK INGESTION")
    logger.info("=" * 60)
    logger.info(f"File: {pricebook_path}")
    ...
    try:
        ...
        logger.error(f"Failed to ingest pricebook: {e}")
        print(f"\n❌ Error: {e}")
        raise
