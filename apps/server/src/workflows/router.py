...
@router.post("/monitored-call", response_model=CallResponse, status_code=201)
async def create_monitored_call(
    request: CallRequest,
    current_user: User = Depends(get_current_user),
    workflow: CallMonitoringWorkflow = Depends(get_call_monitoring_workflow),
) -> CallResponse:
    """
    Create an outbound call with monitoring and CRM integration.

    This workflow endpoint orchestrates:
    1. Creating the call via Voice AI provider
    2. Starting background call monitoring
    3. Updating CRM with call results when complete

    Args:
        request: The call request with phone number and context
        current_user: The authenticated user
        workflow: The call monitoring workflow from dependency injection
    ...
    """
    result = await workflow.create_and_monitor_call(
        request=request,
