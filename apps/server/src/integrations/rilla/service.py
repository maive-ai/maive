...
class RillaService:
    ...
    async def export_conversations(
        ...
        try:
            ...
            logger.info(
                f"Successfully exported {len(result.conversations)} conversations"
            )
            return result
    ...
    async def export_all_conversations(
        ...
        try:
            logger.info(
                f"Exporting ALL conversations from {request.from_date} to {request.to_date}"
            )
            ...
            logger.info(
                f"Found {first_result.total_conversations} conversations across {first_result.total_pages} pages"
            )
            ...
            logger.info(
                f"Fetching {len(remaining_requests)} additional pages concurrently"
            )
            remaining_tasks = [
            ...
            all_conversations: list[Conversation] = list(first_result.conversations)
            for result in remaining_results:
                all_conversations.extend(result.conversations)

            logger.info(
                f"Successfully collected {len(all_conversations)} conversations from all pages"
            )
    ...
    async def export_teams(self, request: TeamsExportRequest) -> TeamsExportResponse:
        ...
        try:
            logger.info(
                f"Exporting teams from {request.from_date} to {request.to_date}"
            )
    ...
    async def export_users(self, request: UsersExportRequest) -> UsersExportResponse:
        ...
        try:
            logger.info(
                f"Exporting users from {request.from_date} to {request.to_date}"
            )
