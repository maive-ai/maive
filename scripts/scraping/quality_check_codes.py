    ...
    try:
        ...
        if result["section_count"] == 0:
            ...
            if result["leaf_node_count"] > 0:
                leaf_quality = (
                    result["leaf_nodes_with_html"] / result["leaf_node_count"]
                ) * 100
                ...
                html_percentage = (
                    result["sections_with_html"] / result["section_count"]
                ) * 100
                error_percentage = (
                    result["sections_with_errors"] / result["section_count"]
                ) * 100
...
def print_quality_report(results: List[dict], organized: dict):
    ...
    print(
        f"  With HTML: {total_with_html:,} ({(total_with_html / total_sections * 100) if total_sections else 0:.1f}%)"
    )
    print(
        f"  With errors: {total_with_errors:,} ({(total_with_errors / total_sections * 100) if total_sections else 0:.1f}%)"
    )
    ...
    for status in status_order:
        ...
        if files:
            ...
            print(
                f"{symbol} {status:8s}: {count:4d} files ({pct:5.1f}%) | "
    ...
    for scraper in sorted(by_scraper.keys()):
        ...
        print(
            f"  Sections: {sections:,} total, {with_html:,} with HTML ({html_pct:.1f}%), {with_errors:,} errors ({error_pct:.1f}%)"
        )
    ...
    for state in sorted(by_state.keys()):
        ...
        print(
            f"{state:4s}: {count:3d} files (✓{good:3d} ⚠{partial:3d} ✗{bad:3d}) | "
    ...
    sorted_by_html_pct = sorted(
        ...
        key=lambda r: (r["sections_with_html"] / r["section_count"])
        if r["section_count"]
        else 0,
    )
    ...
    for r in sorted_by_html_pct[:10]:
        html_pct = (
            (r["sections_with_html"] / r["section_count"] * 100)
            if r["section_count"]
            else 0
        )
        leaf_info = ""
        ...
        print(
            f"  {r['state']}/{r['file']:45s} | {r['scraper']:15s} | "
    ...
    good_files = [
        ...
        and (r["sections_with_html"] / r["section_count"]) > 0.8
    ]
...
def save_quality_report(results: List[dict], organized: dict) -> Path:
    ...
    report = {
        ...
        "summary": {
            ...
            "html_percentage": (total_with_html / total_sections * 100)
            if total_sections
            else 0,
            "error_percentage": (total_with_errors / total_sections * 100)
            if total_sections
            else 0,
        },
...
def load_all_expected_jurisdictions(
...
def print_coverage_report(state_filter: str = None):
    ...
    print(f"\n{'=' * 80}")
    print(
        f"COVERAGE REPORT{f' FOR {state_filter.upper()}' if state_filter else ' (ALL STATES)'}"
    )
    print(f"{'=' * 80}\n")
    ...
    for state in states_to_process:
        ...
        coverage_pct = (
            (total_scraped / total_expected * 100) if total_expected > 0 else 0
        )
        ...
        for scraper_type in sorted(scrapers_expected.keys()):
            ...
            scraper_cov_pct = (
                (scraped_for_scraper / expected_count * 100)
                if expected_count > 0
                else 0
            )

            status = (
                "✓" if scraper_cov_pct >= 95 else "⚠" if scraper_cov_pct >= 80 else "✗"
            )
            print(
                f"  {status} {scraper_type:30s}: {scraped_for_scraper:3d}/{expected_count:3d} "
...
def print_poor_quality_files(results: List[dict], state_filter: str = None):
    ...
    poor_files = [
        r
        for r in results
        if r["status"] in ["PARTIAL", "BAD"]
        and (state_filter is None or r["state"] == state_filter)
    ]
    ...
    print(
        f"{'File':<50} | {'Scraper':<20} | {'Status':<8} | {'HTML %':>8} | {'Sections':>12}"
    )
    ...
    for f in poor_files:
        ...
        print(
            f"{filename:<50} | {f['scraper']:<20} | {status_icon} {f['status']:<6} | {f['html_pct']:>7.1f}% | {sections_str:>12}"
        )
...
def find_missing_jurisdictions() -> Dict[str, Dict[str, List[Dict]]]:
    ...
    for state_dir in CITY_LISTS_DIR.iterdir():
        ...
        print(
            f"  {state.upper()}: {len(scraped_in_state)} jurisdictions currently scraped"
        )
    ...
    for file_info in bad_files:
        ...
        if file_path.exists():
            if dry_run:
                ...
                html_pct = (
                    file_info["sections_with_html"] / file_info["section_count"] * 100
                    if file_info["section_count"] > 0
                    else 0
                )
                print(
                    f"  Would delete: {state}/{filename} "
...
def compare_snapshots(baseline_dir: Path, current_dir: Path = None):
    ...
    print(
        f"    Baseline: {b_sum['total_with_html']:,} ({b_sum['html_percentage']:.1f}%)"
    )
    print(
        f"    Current:  {c_sum['total_with_html']:,} ({c_sum['html_percentage']:.1f}%)"
    )
...
def main():
    ...
    parser.add_argument(
        "--states", type=str, nargs="+", help="Limit to specific states (for snapshots)"
    )
    parser.add_argument(
        "--state",
        type=str,
